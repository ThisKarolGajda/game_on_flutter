  ...
  buildPlayersQuickAccess() {
    return BlocLoadingWidget<UserBloc, UserState>(
      ...
      builder: (context, state) {
        ...
        List<UserModelUserId> players =
            loaded.serverModel.info?.onlinePlayers ?? [];
        return SizedBox(
          height: players.isEmpty ? 0 : 7.5.h,
          child: ListView.builder(
            itemBuilder: (context, index) {
              UserModelUserId userId = players[index];

              return GestureDetector(
                onTap: () async {
                  UserModel? user = await get<GameonUserApi>().getUserFromUserId(loaded.serverModel.basicData.address, loaded.serverModel.basicData.port, userId);
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    children: [
                      MinecraftHeadCacheWidget(userId: userId),
                      Text(
                        userId.nickname,
                        maxLines: 1,
